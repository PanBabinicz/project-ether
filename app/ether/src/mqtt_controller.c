  ...
  switch ((esp_mqtt_event_id_t)event_id) {
    ...
    case MQTT_EVENT_ERROR:
      ...
      if (event->error_handle->error_type == MQTT_ERROR_TYPE_TCP_TRANSPORT) {
        mqtt_controller_log_error_if_nonzero("reported from esp-tls", event->error_handle->esp_tls_last_esp_err);
        mqtt_controller_log_error_if_nonzero("reported from tls stack", event->error_handle->esp_tls_stack_err);
        mqtt_controller_log_error_if_nonzero("captured as transport's socket errno",  event->error_handle->esp_transport_sock_errno);
